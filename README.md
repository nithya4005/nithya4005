<div align="center">

# hi, i'm nithya 👋

*3rd year Software Engineering @ McMaster*

![ML/AI](https://img.shields.io/badge/ML%20%2F%20AI-%23a78bfa?style=flat-square&logoColor=white)
![Open Source](https://img.shields.io/badge/Open%20Source-%23f472b6?style=flat-square&logoColor=white)
![Web Dev](https://img.shields.io/badge/Web%20Dev-%23a78bfa?style=flat-square&logoColor=white)
![Software Eng](https://img.shields.io/badge/Hardware%20Tinkerer-%23f472b6?style=flat-square&logoColor=white)

</div>

---

### about me

- 🍳 i love cooking. my food does *not* love me back. the experimentation continues regardless.
- 🎲 obsessed with Catan - so much that i built a full [Catan game](#) with AI players that actually know what they're doing (unlike my human opponents).
- 🌍 happiest when travelling somewhere new or doing something genuinely chaotic and adventurous.

---

### what i build

`ml / ai` `open source` `data pipelines` `web dev` `llms` `tooling`

---

### stack

`python` `react` `pytorch` `fastapi` `huggingface` `docker` `postgres`

---

### currently building

```
// in progress
▸ a custom MCP connector syncing Notion, Microsoft To-Do & Google Calendar —
  an AI scheduling assistant that adapts to how humans actually live
▸ small hardware projects —  building small useful things i can hold
▸ going down the LLM rabbit hole — understanding the internals, not just the APIs
```

---
